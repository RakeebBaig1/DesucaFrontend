...
import { AddTask, AssignTask, changeManager, DeleteTask, DeleteTask2, GetAllTask, Getmanager, GetUsers, UpdateDescription, UpdateDescription2, UpdateStatus, UpdateStatus2 } from "@/app/Requests/req";
...
const StylishCardList = (props: any) => {
  ...
  const id = searchParams.get('user'); // Get query parameters
  ...
  React.useEffect(() => {
    const fetchData = async () => {
        try {
          ...
          let res3 = await GetAllTask(global.id)
  ...
  const handleSubmit = async () => {
    ...
    setTask(tt2?.data)
      console.log('Api response Assign new Task ',reso)
  ...
  const  handleDelete = async (dd: any) => {
    ...
    let rr = await DeleteTask2(global.id,dd)
  ...
  const handleEditSave = () => {
    ...
    prevItems.map(async (item) => {
        if (item._id === editItem) {
           ...
           let ress = await UpdateDescription2(global.id, item._id, editedDescription);
    ...
    else
{
  ...
  const handleStatusToggle = async (dd: any) => {
    ...
    let aa=await UpdateStatus2(global.id, dd, matchedstat);
  ...
  return (
    <div className="flex-1 gap-4 justify-center bg-white p-2 h-screen space-y-5">
      <div className="bg-slate-500 w-screen h-20 -ml-2  flex  flex-col p-2">
         <Button
        variant="contained"
        color="primary"
        startIcon={<AddIcon />}
    ...
    "& .MuiDialog-paper": {
      ...
      {task?.map((item) => (
        <Card
          key={item._id}
          className=" md:w-full sm:w-28 lg:w-full p-4 shadow-lg rounded-lg x-3 pt-2"
          sx={{
